...
class ResultController extends Controller
{
    public function result(Request $request)
    {
        ...
        $results = DB::select(
            "SELECT
              TO_CHAR(updated_at, 'YYYY-MM') as regist_time,
              COUNT(*) as count
          FROM
              tasks
          WHERE state_id = 4
    ...
    public function day(Request $request, $yearMonth)
    {
        ...
        // postgreSql
        $results = DB::select(
            "SELECT t.id AS task_id, u.name AS user_name, t.task_name, t.task_body, t.img1, t.img2, t.img3, TO_CHAR(t.updated_at, 'YYYY-MM') AS ym, TO_CHAR(t.updated_at, 'DD日 HH24:MI') AS dayTime
            FROM tasks t
            LEFT JOIN users u ON t.user_id = u.id
            WHERE t.user_id = :authId
            AND state_id = 4
            AND group_id = 0
            AND TO_CHAR(t.updated_at, 'YYYY-MM') = :yearMonth
            ORDER BY t.updated_at DESC
            ",
            ['authId' => $auth_user_id, 'yearMonth' => $yearMonth]
        );
